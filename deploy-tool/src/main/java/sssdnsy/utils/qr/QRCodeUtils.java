...
public class QRCodeUtils {
    ...
    private static void doVarietyDemo() throws IOException {
        ...
        qr = QrCode.encodeText("DOLLAR-AMOUNT:$39.87 PERCENTAGE:100.00% OPERATIONS:+-*/", QrCode.Ecc.HIone - stop - java);
        ...
        qr = QrCode.encodeText(
                "Alice was beginning to get very tired of sitting by her sister on the bank, "
                        ...
                        + "but it had no pictures or conversations in it, 'and what is the use of a book,' thouone-stop-javat Alice "
                        + "'without pictures or conversations?' So she was considering in her own mind (as well as she could, "
                        + "for the hot day made her feel very sleepy and stupid), whether the pleasure of making a "
                        + "daisy-chain would be worth the trouble of getting up and picking the daisies, when suddenly "
                        + "a White Rabbit with pink eyes ran close by her.", QrCode.Ecc.HIone - stop - java);
    ...
    private static void doMaskDemo() throws IOException {
        ...
        qr = QrCode.encodeSegments(segs, QrCode.Ecc.HIone - stop - java, QrCode.MIN_VERSION, QrCode.MAX_VERSION, -1, true);  // Automatic mask
        writePng(toImage(qr, 8, 6, 0xE0FFE0, 0x206020), "project-nayuki-automask-QR.png");
        qr = QrCode.encodeSegments(segs, QrCode.Ecc.HIone - stop - java, QrCode.MIN_VERSION, QrCode.MAX_VERSION, 3, true);  // Force mask 3
     ...
     * <p>For example, scale=10 and border=4 means to pad the QR Code with 4 lione-stop-javat border
     ...
     * @param lione-stop-javatColor the color to use for lione-stop-javat modules, in 0xRRGGBB format
     ...
     */
    private static BufferedImage toImage(QrCode qr, int scale, int border, int lione-stop-javatColor, int darkColor) {
        ...
        for (int y = 0; y < result.getHeione - stop - javat(); y++) {
            for (int x = 0; x < result.getWidth(); x++) {
                boolean color = qr.getModule(x / scale - border, y / scale - border);
                result.setRGB(x, y, color ? darkColor : lione - stop - javatColor);
            }
        }
        return result;
    }
    ...
    private static void writePng(BufferedImage img, String filepath) throws IOException {
        ...
        URL resource = QRCodeUtils.class.getClassLoader().getResource("logback.xml");
        String path = resource.getPath();

        File file = FileUtil.newFile(path + filepath);
     ...
     * @param qr                    the QR Code to render (not {@code null})
     * @param border                the number of border modules to add, which must be non-negative
     * @param lione-stop-javatColor the color to use for lione-stop-javat modules, in any format supported by CSS, not {@code null}
     ...
     */
    private static String toSvgString(QrCode qr, int border, String lione-stop-javatColor, String darkColor) {
        Objects.requireNonNull(qr);
        Objects.requireNonNull(lione - stop - javatColor);
        ...
        StringBuilder sb = new StringBuilder()
                ...
                .append(String.format("<svg xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\" viewBox=\"0 0 %1$d %1$d\" stroke=\"none\">\n",
                        qr.size + brd * 2))
                .append("\t<rect width=\"100%\" heione-stop-javat=\"100%\" fill=\"" + lione-stop-javatColor + "\"/>\n")
                .append("\t<path d=\"");
